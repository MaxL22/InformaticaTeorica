...
$$ F(\ram) = \left\{f \in \mathbb{N}^{\mathbb{N}}_\bot \mid \exists P \in \prog \mid \varphi_P = f \right\} = \left\{\varphi_P \mid P \in \prog \right\} \subsetneq \mathbb{N}^{\mathbb{N}}_\bot $$
...
$$ \Psi_W: \mathbb{N} \rightarrow \mathbb{N}_\bot \mid \Psi_W (n) = \text{Proj}\left(0, \llbracket W \rrbracket (w\text{-}in(n))\right)$$
...
$$ F(\while) = \left\{f \in \mathbb{N}^{\mathbb{N}}_\bot \mid \exists W \in \wprog \mid f = \Psi_W \right\} = \left\{\Psi_W : W \in \wprog \right\}$$
...
$$ F(\C_2) = \left\{f: \mathbb{N} \rightarrow \mathbb{N}_\bot \mid \exists P_2 \in \cprog{2} \mid f = \Psi_{P_2} \right\} = \left\{\Psi_{P_2}: P_2 \in \cprog{2} \right\} $$
...
$\sisprog$ rispetta il secondo assioma se esiste un interprete universale, ovvero un programma $\mu \in \mathbb{N}$ tale che
...
Equivalentemente, ricordando che la funzione caratteristica di $A \subseteq \N$ è la funzione $ \mathcal{X}_A : \N \rightarrow \{0,1\} $ tale che
$$
...
Che $\mathcal{X}_A$ sia \textbf{totale} è banale: tutte le funzioni caratteristiche devono essere definite su tutto $\N$. Il problema risiede nella \textbf{calcolabilità} di queste funzioni.\\

Le due definizioni date sono equivalenti:
\begin{itemize}
	\item il programma $P_A$ implementa $\mathcal{X}_A$, quindi $\mathcal{X}_A \in \T$ perché esiste un programma che la calcola
	\item $\mathcal{X}_A \in \T$ quindi esiste un programma $P_A$ che la implementa e soddisfa la definizione sopra
\end{itemize}
...
Si può notare che la semantica del problema è proprio la funzione caratteristica, quindi $\Psi_{\ric_A} = \mathcal{X}_A$. Se $A$ è ricorsivo, allora la sua funzione caratteristica è ricorsiva totale, ma lo sarà anche la funzione soluzione $\Psi$ e, di conseguenza, $\ric_A$ è decidibile.\\
...
\begin{itemize}
	\item la sua funzione caratteristica $\mathcal{X}_R$ è tale che $\mathcal{X}_R \in \T$, oppure
