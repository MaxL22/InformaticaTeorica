...
\begin{itemize}
	\item $\forall i \in \N^+$, $A_i \neq \emptyset$
	\item $\forall i,j \in \N^+$, se $i \neq j$ allora $A_i \cap A_j = \emptyset$
	\item $\bigcup_{i \in \N^+} A_i = A$
\end{itemize}
...
$$ J_n = \begin{cases}
	...
	\{1, \dots , n\} & \text{ se } n > 0 \\
\end{cases}$$
Un insieme $A$ ha \textbf{cardinalità finita} sse $A \sim J_n$ per qualche $n \in \N$; in tal caso possiamo scrivere $|A| = n$. La classe di equivalenza $[J_n]_{\sim}$ identifica tutti gli insiemi di $\U$ contenenti $n$ elementi.\\
...
Un insieme $A$ è numerabile sse $A \sim \N$, ovvero $A \in [\N]_\sim$. Vengono anche detti \textbf{listabili}, in quanto è possibile elencare tutti gli elementi dell'insieme $A$ tramite una funzione $f$ biettiva tra $\N$ e $A$; grazie ad $f$ possiamo elencare gli elementi di $A$, formando l'insieme 
$$ A = \{f(0), f(1). \dots \} $$
...
Gli insiemi non numerabili sono insiemi a cardinalità infinita ma non listabili, sono "più fitti" di $\N$; ogni lista generata non può essere esaustiva.\\
...
\begin{proof}
	...
	Continuiamo dimostrando che $\N \nsim (0,1)$: serve dimostrare che l'intervallo $(0,1)$ non è listabile, quindi che ogni lista manca di almeno un elemento. Proviamo a "costruire" un elemento che andrà a mancare. Per assurdo, sia $\N \sim (0,1)$, allora possiamo listare gli elementi di $(0,1)$ come 
	$$ 
	...
	Il terzo punto $\mathbb{R} \nsim \N$ si dimostra per transitività.\\
...
\begin{proof}
	...
	Costruiamo una funzione $\varphi: \N \rightarrow \N_\bot$ per dimostrare l'assurdo. Un'idea potrebbe essere $\varphi(n) = f_n(n) + 1$, "spostando" la diagonale, ma non tiene in considerazione il caso $f_n (n) = \bot$ in quanto non sapremmo dare un valore a $\varphi (n) = \bot +1$. Definiamo quindi
	$$ 
...
Grazie a questo è possibile dimostrare anche che $\mathbb{Q} \sim \N$, infatti i numeri razionali si possono rappresentare come coppie (num, den) e, in generale, tutte le tuple sono isomorfe e $\N$, basta iterare in qualche modo la funzione coppia di Cantor.\\
...
$$ f: \dati \rightarrow \dati \;\; \text{ con funzioni } \;\; f': \N \rightarrow \N_\bot $$

In altre parole, l'universo dei problemi per i quali cerchiamo una soluzione automatica è rappresentabile da $\N_\bot^{\N}$ e di conseguenza $\dati \sim \N$.\\

\section{$\prog \sim \N$}
Adesso lavoriamo sulla parte della relazione che afferma 
$$ F(\C) \sim \prog \sim \N $$

Ovvero, la potenza computazionale (l'insieme dei programmi che un sistema di calcolo $\C$ riesce a calcolare, $F(\C)$) è isomorfa all'insieme di tutti i programmi, a loro volta isomorfi a $\N$.\\
...
\begin{enumerate}
	...
	$$
\end{enumerate}
Con $\varphi_P: \N \rightarrow \N_\bot$ indichiamo la semantica del programma $P$.\\
...
$$ F(\ram) = \left\{f \in \N^{\N}_\bot \mid \exists P \in \prog \mid \varphi_P = f \right\} = \left\{\varphi_P \mid P \in \prog \right\} \subsetneq \N^{\N}_\bot $$
L'insieme è formato da tutte le funzioni $f: \N \rightarrow \N_\bot$ che hanno un programma che le calcola in un sistema $\ram$.\\
...
$$
Ar^{-1} (n) = \begin{cases}
	R_{\frac{n}{3}} \leftarrow R_{\frac{n}{3}} + 1 & \text{ se } n \equiv 0 \mod 3 \\
	R_{\frac{n-1}{3}} \leftarrow R_{\frac{n-1}{3}} \dotminus 1 & \text{ se } n \equiv 1 \mod 3 \\
	\text{\texttt{if} } R_{\sin\left(\frac{n+1}{3}\right)} = 0 \text{ \texttt{then goto} } \des\left(\frac{n+1}{3}\right) & \text{ se } n \equiv 2 \mod 3 \\
\end{cases}
$$
...
Abbiamo quindi dimostrato che $\prog \sim \N$.\\
...
L'insieme, grazie alla dimostrazione di $\prog \sim \N$, è numerabile.\\
...
$$ \wprog = \{\prog \text{ scritti in linguaggio } \while \} $$
...
$$ \Psi_W: \N \rightarrow \N_\bot \mid \Psi_W (n) = \text{Proj}\left(0, \llbracket W \rrbracket (w\text{-}in(n))\right)$$
...
$$ F(\while) = \left\{f \in \N^{\N}_\bot \mid \exists W \in \wprog \mid f = \Psi_W \right\} = \left\{\Psi_W : W \in \wprog \right\}$$
...
$$ F(\C_2) = \left\{f: \N \rightarrow \N_\bot \mid \exists P_2 \in \cprog{2} \mid f = \Psi_{P_2} \right\} = \left\{\Psi_{P_2}: P_2 \in \cprog{2} \right\} $$
...
$$ \forall P_1 \in \cprog{1} \mid f = \Psi_{P_1} \implies \exists P_2 \in \cprog{2} \mid f = \Psi_{P_2} $$
...
\begin{itemize}
	...
	\item $g_1, \dots, g_k: \N^k \rightarrow \N$ "funzioni intermedie"
	\item $\underline{x} \in \N^k$ input
\end{itemize}
Allora definiamo 
$$ \com (h, g_1, \dots, g_k): \N^k \rightarrow \N $$
...
\begin{itemize}
	\item $\forall f \in \elem \implies f \in \ricprim$
	\item se $h, g_1, \dots, g_k \in \ricprim \implies \com(h, g_1, \dots, g_k) \in \ricprim$
	\item se $g,h, \in \ricprim \implies \rp (g,h) \in \ricprim$
...
\end{theor}
\begin{proof}
	\textbf{Passo Base:} Le funzioni di $\elem$ sono ovviamente while-programmabili, mostrato in precedenza.\\
	...
	Un programma $\while$ che calcola $\com$ è
		...
		\begin{minipage}{0.85\textwidth}
			\begin{tcolorbox}[colback=white,sharp corners,boxrule=.3mm]
				\begin{algorithm}[H]
					...
					\SetAlgoNoEnd
					\SetKwSty{texttt}
					\tcp{Inizialmente in $x_1$ ho $\langle x,y \rangle$}
					input($x,y$) \\
					\Begin{
						$t:= G(x)$; \hfill \tcp{$t$ contiene $f(x,y)$} 
						$k:= 1$; \\
						\While{$k \leq y$}{
							\Begin{
								$t:= H(t, k-1, x)$; \\
...
Il $\for$ language è quindi un linguaggio $\while$ dove l'istruzione di loop è un \texttt{for}. Possiamo quindi dire che $\for = \ricprim$, quindi che $F(\for) \subset F(\while)$.\\
...
\mathcal{A} (m,n) = \begin{cases}
	...
	\mathcal{A}(m-1, A(m, n-1)) & \text{ se } m > 0 \wedge n > 0
\end{cases}
$$
...
Introduciamo un operatore per permettere la presenza di funzioni parziali: \textbf{minimalizzazione}. Sia $f: \N^{n+1} \rightarrow \N$ con $f(\underline{x}, y)$ e $\underline{x} \in \N^n$, allora: 
$$
...
\begin{proof}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item sia $f \in \cp$ while-programmabile per I.H., mostro che $\MIN(f)$ è while-programmabile. Devo trovare un programma $\while$ che calcoli la minimizzazione:
			\begin{center}
				\begin{minipage}{.85\textwidth}
					\begin{tcolorbox}[colback=white,sharp corners,boxrule=.3mm]
						\begin{algorithm}[H]
				...
				\end{minipage}
			\end{center}
			Questo è un programma che calcola la minimizzazione: se non esiste $y$ che azzeri $f(\underline{x}, y)$ il programma va in loop (i.e., incrementa di 1 a ogni iterazione e termina quando azzera la funzione), quindi la semantica di $P$ è $\bot$ secondo $\MIN$
		\end{itemize}
...
\begin{proof}
	...
	La funzione di stato prossimo $\llbracket \rrbracket (): \N^{21} \rightarrow \N^{21}$ restituisce elementi in $\N^{21}$, mentre gli elementi in $\cp$ hanno codominio $\N$. \\
...
\begin{itemize}
	...
	\item $\T$ insieme delle \textbf{funzioni ricorsive totali}
\end{itemize}
...
\begin{proof}
	...
	\begin{enumerate}
		\item $\{\varphi_i\} = \cp$
		\item $\exists u: \varphi_u (\langle x,y \rangle) = \varphi_n (x)$
		\item $\exists S_1^1 \in \T: \varphi_n (\langle x,y \rangle ) = \varphi_{Ŝ^1_1 (e,i)} (x)$
	\end{enumerate}
	
	Voglio trovare un compilatore $t \in \T$ che sia corretto. Ma allora
	$$ \varphi_i (x) \stackrel{(2)}{=} \varphi_u (\langle x,y \rangle ) \stackrel{(1)}{=} \Psi_e (\langle x,i \rangle) \stackrel{(3)}{=} \Psi_{S_1^1(e,i)} (x) $$
...
\begin{coroll}
	Dati gli SPA $A,B,C$ esiste sempre un compilare da $A$ a $B$ scritto nel linguaggio $C$.
\end{coroll}
\begin{proof}
...
La risposta alla prima domanda è \si\ per $\ram$, ma lo è in generale per tutti i SPA che ammettono una codifica per i propri programmi.\\
...
Ad esempio, dato un SPA $\{\varphi_i\}$ ci chiediamo se
...
\begin{enumerate}
	...
	\item So che $S^1_1 (e,n)$ è una funzione ricorsiva totale
...
Non bisogna esibire una struttura come risultato (cosa che accade nei problemi di ricerca od ottimizzazione), ma solo una risposta \si/\no.\\
...
\begin{proof}
	...
	Sia $\alpha \in \N$ la codifica del programma $A$, allora $\varphi_\alpha = f$. Valutiamo $\varphi_\alpha$ in $\alpha$:
	$$ 
	...
	\begin{itemize}
		\item nel primo caso ho $\varphi_\alpha (\alpha) = 0$ se non termina, ma è una contraddizione perché $A \in \T$
...
\begin{proof}
	...
	Possiamo notare che $\Phi_{\ar} (x,x) = \Phi_{\arph} (x,x)$, quindi nel caso $x = y$ il problema $\ar$ coincide con il $\arph$. Il risultato del teorema precedente dimostra che $\arph$ è indecidibile, quindi anche $\ar$ è indecidibile.\\
\end{proof}
...
\begin{itemize}
	...
	\item \textbf{clock} per contare i passi di interpretazione
	\item \textbf{check del clock} per controllare l'arrivo alla quota $y$
\end{itemize}
...
In breve: si aggiunge una variabile che conta i passi, se l'esecuzione supera $y$ restituisce 0, altrimenti 1.\\
...
$$ B = \left\{x \in \N \mid \exists y \in \N \mid \left(xR_{\hat{P}}y\right)\right\} $$
...
Se avessi indicazioni sulla monotonia della routine $F$ si avrebbe sicuramente un insieme ricorsivo. In questo caso non assumiamo niente, quindi si rimane negli insiemi ricorsivamente numerabili.\\
...
Si può comparare a un \textit{libro con infinite pagine}, su ognuna delle quali compare un elemento di $A$; il programma non fa altro che sfogliare le pagine.\\
...
\begin{theor}
	...
	\begin{enumerate}
		\item $A$ è ricorsivamente numerabile, con $A = \Im_f$ e $f \in \T$ funzione ricorsiva totale
...
\begin{proof}
	...
	$3 \implies 1$: Sappiamo che $A = \left\{x \in \N \mid \tc (x,y) \in R\right\}$, con $R$ relazione ricorsiva. \\
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item se $x = \sin(n)$ con $n = \langle x,y \rangle$ per quale $y$ tale che $(x,y) \in R$ allora $x \in A$ per definizione di $A$
		\end{itemize}
	\end{itemize}
...
non è ricorsivo, altrimenti il problema dell'arresto sarebbe decidibile. \\

Tuttavia, questo insieme è ricorsivamente numerabile: infatti il programma $P$:
...
$$ 
\varphi_P (x) = \begin{cases}
	1 & \text{ se } \varphi_U (x,x) = \varphi_x (x) \downarrow \\
	\bot & \text{ altrimenti}
\end{cases}
$$
...
\end{theor}
\begin{proof}
	Se $A$ ricorsivo, esiste un programma $P$ in grado di riconoscerlo, ovvero un programma che restituisce $1$ se $x \in A$, $0$ altrimenti.\\
...
\begin{proof}
	...
	\begin{center}
		\begin{minipage}{0.45\textwidth}
			$$ A \cap B = \Dom_{\varphi_{P' \in \cp}}$$
		\end{minipage}
		\begin{minipage}{0.45\textwidth}
			$$ A \cup B = \Dom_{\varphi_{P'' \in \cp}}$$
		\end{minipage}
	\end{center}
...
Il \textbf{teorema di Rice} permette di mostrare che gli insiemi appartenenti a una certa classe non sono ricorsivi.\\
...
\begin{proof}
	Sia $I$ insieme che rispetta le funzioni con $I \neq \emptyset$ e $I \neq \N$. Assumiamo, per assurdo, che $I$ sia ricorsivo.\\
	...
	Sappiamo che $t \in \T$ dato che è calcolabile dal programma $P$
	\begin{center}
		\begin{minipage}{.45\textwidth}
			\begin{tcolorbox}[
				colback=white,
...
Ma allora $\pc$ non è ricorsivo. Dato ciò, la correttezza dei programmai non può essere testata automaticamente. Esistono, però, dei casi limite in cui è possibile costruire dei test automatici: 
\begin{itemize}
