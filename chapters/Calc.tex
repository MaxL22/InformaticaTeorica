...
\begin{proof}
	...
	$$ P \in \prog \;\; | \;\; \varphi_P (\langle x,y \rangle) = x + y $$
...
\begin{proof}
	...
	$$ \exists m \in \N \;\; | \;\; \varphi_m (i) = t\left(S_1^1 (e,i)\right) $$
...
a='a=%r;print(a%%a)';print(a%a)
	...
	$$ \exists j \in \N \;\; | \;\; \varphi_j (x) = j \text{ per ogni input } x \in \N \text{?}$$
...
$$ \exists j \in \N \;\; | \;\; \varphi_j (x) = \varphi_{Z (j)} (x) = j $$

Quindi effettivamente esiste un programma $j$ la cui semantica è proprio quella di stampare se stesso.\\

La risposta alla prima domanda è \textit{sì} per $\ram$, am lo è in generale per tutti i SPA che ammettono una codifica per i propri programmi.\\
...
Per il teorema di ricorsione
$$ (\ast \ast) \quad \quad \exists i \in \N \;\; | \;\; \varphi_i = \varphi_{g(i)} $$

Unendo $(\ast)$ e $(\ast \ast)$ otteniamo 
$$ \exists i \in \N \;\; | \;\; \Psi_{t(i)} \stackrel{(\ast)}{=} \varphi_{g(i)} \stackrel{(\ast \ast)}{=} \varphi_i \quad \forall t \in \T $$

Di conseguenza, la risposta alla seconda domanda è \textit{no}.\\
